...
//        }
        ...
        public DeferredResult<String> restAsync2(int idx) throws InterruptedException {
            ...
            dr.setResult(restTemplate.getForObject("http://localhost:8081/service?req={req}", String.class, "hello" + idx));
            return dr;
        }
        ...
        public DeferredResult<String> restAsyncDeferred3(int idx) throws InterruptedException {
            ...
            f1.addCallback(s -> {
                ListenableFuture<ResponseEntity<String>> f2 = asyncRestTemplate.getForEntity(URL2, String.class, s.getBody()
                );
                f2.addCallback(s2 -> {
                    ListenableFuture<String> f3 = myService.work(s2.getBody());
                    f3.addCallback(s3 -> {
        ...
        public DeferredResult<String> restAsyncChain(int idx) throws InterruptedException {
            DeferredResult<String> dr = new DeferredResult<>();
            dr.setResult(restTemplate.getForObject("http://localhost:8082/r1restAsync?idx={idx}", String.class, idx));
            return dr;
        }
